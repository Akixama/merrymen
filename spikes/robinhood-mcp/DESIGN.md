...
| **Fix `created_at` first (prerequisite)** | DDL declares `INTEGER` (`store.ts:54`) but every caller writes `new Date().toISOString()` (`index.ts:1149`) | SQLite sorts TEXT above INTEGER, so `created_at > unixepoch()-86400` windows (`store.ts:483`,`515`) already match **all history, not 24h** (empirically confirmed). Settlement lag makes a correct integer time key load-bearing, not cosmetic — reconciliation of late fills is untrustworthy until this is fixed |
...
| # | Step | Value | Blocked? |
|---|---|---|---|
| 0 | **Fix `created_at` to a real unix int** (`store.ts:54` / all `addTrade` callers) | Correct 24h budget windows on *both* rails — a real pre-existing bug | No |
